# modified version of https://github.com/rythei/PyTorchOT/blob/master/ot_pytorch.py
...
def sinkhorn(M, reg, numItermax=1000, stopThr=1e-9, cuda = True):
...
def sinkhorn_stabilized(M, reg, numItermax=1000, tau=1e2, stopThr=1e-9, warmstart=None, print_period=20, cuda=True):
...
# pairwise distance, tons of algorithms for this
def earth_movers_distance(x, y, method='l1'):
    n = x.size()[0]
    m = y.size()[0]
    d = x.size()[1]
